...
3. Write the spec using the template below, then publish it to the **local tracker**: write `.scratch/<map-slug>/spec.md` — where `<map-slug>` is the effort's slug from `map.md`'s Notes (or derive it from the branch name). Run on the effort's `feature/<map-slug>` branch; the spec is tracked with the effort. No triage label is needed — the local tracker carries no labels.
