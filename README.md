...
被魔改的 skill 路径从 `./skills/engineering/<name>` 替换为 `./skills/edenxiang/<name>`（原版文件保留在仓库供同步对照）。插件安装即魔改版生效。
...
- 魔改 skill 在 `.claude-plugin/plugin.json` 中**路径替换**为 `./skills/edenxiang/<name>`（原版文件保留）；`xhon-workflows` 等自创 skill 不进 plugin.json，靠 link-skills.sh 链接生效
